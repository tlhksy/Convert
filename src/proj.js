...
/* Meridian arc distance, series carried to e^10.
   The classical e^6 truncation used in Snyder (1987) leaves a latitude
   dependent residual of up to 0.19 mm across 35-43 N; extending the series
   removes it. Coefficients follow the standard expansion of the rectifying
   latitude. */
function meridianArc(a, e2, phi) {
  var e4 = e2 * e2, e6 = e4 * e2, e8 = e6 * e2, e10 = e8 * e2;
  var c0 = 1 - e2 / 4 - 3 * e4 / 64 - 5 * e6 / 256 - 175 * e8 / 16384 - 441 * e10 / 65536;
  var c2 = -(3 * e2 / 8 + 3 * e4 / 32 + 45 * e6 / 1024 + 105 * e8 / 4096 + 2205 * e10 / 131072);
  var c4 = (15 * e4 / 256 + 45 * e6 / 1024 + 525 * e8 / 16384 + 1575 * e10 / 65536);
  var c6 = -(35 * e6 / 3072 + 175 * e8 / 12288 + 3675 * e10 / 262144);
  var c8 = (315 * e8 / 131072 + 2205 * e10 / 524288);
  var c10 = -(693 * e10 / 1310720);
  return a * (c0 * phi + c2 * Math.sin(2 * phi) + c4 * Math.sin(4 * phi)
    + c6 * Math.sin(6 * phi) + c8 * Math.sin(8 * phi) + c10 * Math.sin(10 * phi));
}

/* Footpoint latitude: the latitude whose meridian arc equals M.
   Solved by Newton iteration rather than by the e1 series, so that the
   inverse carries no truncation of its own and stays consistent with
   meridianArc by construction. Converges to double precision in three
   iterations over the relevant latitude band. */
function footpointLat(a, e2, M) {
  var e4 = e2 * e2, e6 = e4 * e2, e8 = e6 * e2, e10 = e8 * e2;
  var c0 = 1 - e2 / 4 - 3 * e4 / 64 - 5 * e6 / 256 - 175 * e8 / 16384 - 441 * e10 / 65536;
  var phi = M / (a * c0);
  for (var i = 0; i < 8; i++) {
    var sp = Math.sin(phi);
    var dMdphi = a * (1 - e2) / Math.pow(1 - e2 * sp * sp, 1.5);
    var dphi = (M - meridianArc(a, e2, phi)) / dMdphi;
    phi += dphi;
